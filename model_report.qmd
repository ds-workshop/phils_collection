...
- The model was trained on `v$metadata$user$split$data |> bind_cols() |> nrow()` games published through `r `v$metadata$user$end_train_year`

- The evaluation dataset used in this model card is `test_data`, which contains games from the following years:
