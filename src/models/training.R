...
prepare_split <- function(collection,
  ...
  tibble(
    username = unique(collection$username),
    split = list(new_split),
    settings = list(tibble(
      end_train_year = end_train_year,
      valid_years = valid_years,
      retrain_years = retrain_years,
      min_ratings = min_ratings
  ...
  recipe %>%
    ...
    step_mutate(
      year = dplyr::case_when(
        yearpublished < 1900 ~ 1900,
        TRUE ~ yearpublished
      ),
      role = "predictor"
    ) %>%
    # indicator for published before 1900
    step_mutate(published_before_1900 = dplyr::case_when(
      yearpublished < 1900 ~ 1,
      TRUE ~ 0
    )) %>%
    # solo game
    # big box/deluxe/anniversary edition
    step_mutate(deluxe_edition = dplyr::case_when(
      grepl("kickstarter|big box|deluxe|mega box", tolower(name)) == T ~ 1,
      TRUE ~ 0
    )) %>%
    ...
    step_mutate(description_from_publisher = dplyr::case_when(
      grepl("description from publisher", tolower(description)) == T ~ 1,
      TRUE ~ 0
    ))
}
...
add_imputation <- function(recipe) {
  recipe %>%
    ...
    step_impute_median(
      ...
      minage,
      time_per_player
    ) %>% # medianimpute numeric predictors
    # truncate minage to no greater than 18
    step_mutate(minage = dplyr::case_when(
      minage > 18 ~ 18,
      minage < 0 ~ 0,
      TRUE ~ minage
    )) %>%
    # truncate player counts
    step_mutate(minplayers = dplyr::case_when(
      minplayers > 10 ~ 10,
      TRUE ~ minplayers
    )) %>%
    step_mutate(maxplayers = dplyr::case_when(
      maxplayers > 20 ~ 10,
      maxplayers <= 1 ~ 1,
      TRUE ~ maxplayers
    )) %>%
...
select_best_wflow = function(wflowset, metric) {
    ...
    wflowset |>
        filter(wflow_id == best_wflow$wflow_id) |>
        pull(wflow_id)
}
